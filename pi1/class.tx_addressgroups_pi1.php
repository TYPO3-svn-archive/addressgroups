...
/**
 * main class for the addressgroups plugin, outputs addresses either by direct
 * selection or by selection via groups or a combination of both
 * 
 * @author Ingo Renner <typo3@ingo-renner.com>
 */
class tx_addressgroups_pi1 extends tslib_pibase {
	var $prefixId      = 'tx_addressgroups_pi1';		// Same as class name
	var $scriptRelPath = 'pi1/class.tx_addressgroups_pi1.php';	// Path to this script relative to the extension dir.
	var $extKey        = 'addressgroups';	// The extension key.
		...
		if(!empty($groupList) && !empty($this->conf['pidList'])) {
			if($this->ffData['combination'] == '0') {
				...
				$res = $GLOBALS['TYPO3_DB']->sql_query(
					...
					'FROM tt_address '. 
					'JOIN tt_address_tx_addressgroups_group_mm AS tta_txagg_mm ON tt_address.uid = tta_txagg_mm.uid_local '. 
					'JOIN tx_addressgroups_group AS txagg ON txagg.uid = tta_txagg_mm.uid_foreign '. 
				...
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
					'DISTINCT tt_address.*',
					'tt_address, tt_address_tx_addressgroups_group_mm, tx_addressgroups_group',
					'tt_address_tx_addressgroups_group_mm.uid_foreign IN('.$groupList.
						') AND tt_address.uid = tt_address_tx_addressgroups_group_mm.uid_local '.
						$this->cObj->enableFields('tt_address').
						' AND tt_address.pid IN('.$this->conf['pidList'].')'
				);
			}
	...
	function getItemMarkerArray($address) {
		...
		if(!empty($address['image'])) {
			...
			$iConf['image.']['file.']['maxW'] = $lConf['image.']['file.']['maxW'];
			$iConf['image.']['file.']['maxH'] = $lConf['image.']['file.']['maxH'];
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/addressgroups/pi1/class.tx_addressgroups_pi1.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/addressgroups/pi1/class.tx_addressgroups_pi1.php']);
}
