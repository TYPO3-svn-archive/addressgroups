 ...
 *
 */
class tx_addressgroups_tceFunc_selectTreeView extends t3lib_treeview {
 ...
 */
class tx_addressgroups_treeview {
			...
			if ($row['sys_language_uid'] && $row['l18n_parent'] ) {
				...
				if($config['treeView'] && $config['foreign_table']) {
					...
					if($config['treeViewClass'] && is_object($treeViewObj = &t3lib_div::getUserObj($config['treeViewClass'],'user_',false))) {
						...
						$treeViewObj = t3lib_div::makeInstance('tx_addressgroups_tceFunc_selectTreeView');
					}
					...
					if ($table == $config['foreign_table']) {
						...
						// get default items
					$defItems = array();
					if (is_array($config['items']) && $table == 'tt_content' && $row['CType']=='list' && $row['list_type']=='addressgroups_pi1' && $field == 'pi_flexform')	{
						reset ($config['items']);
	...
	function findRecursiveGroups ($PA,$row,$table,$storagePid,$treeIds) {
		$errorMsg = array();
		if ($table == 'tt_content' && $row['CType']=='list' && $row['list_type']=='addressgroups_pi1') { // = tt_content element which inserts plugin tx_addressgroups_pi1
		...
		} elseif ($table == 'tx_addressgroups_group' || $table == 'tt_address') {
			if ($table == 'tx_addressgroups_group' && $row['pid'] == $storagePid && intval($row['uid']) && !in_array($row['uid'],$treeIds))	{ // if the selected group is not empty and not in the array of tree-uids it seems to be part of a chain of recursive groups
			...
			if ($table == 'tt_address' && $row['tx_addressgroups_group']) { // find recursive groups in the tt_address db-record
				$rcList = $this->compareGroupVals ($treeIds,$row['tx_addressgroups_group']);
			}
			// in case of localized records this doesn't work
			if ($storagePid && $row['pid'] != $storagePid && $table == 'tx_addressgroups_group') { // if a storagePid is defined but the current group is not stored in storagePid
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/addressgroups/class.tx_addressgroups_treeview.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/addressgroups/class.tx_addressgroups_treeview.php']);
}
?>
